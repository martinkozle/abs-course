...
def build_model(state_space_shape, num_actions, learning_rate):
    ...
    model.add(Conv2D(3, 3, activation='relu', input_shape=state_space_shape))
    model.add(MaxPool2D(pool_size=(2, 2)))
    model.add(Conv2D(3, 5, activation='relu'))
    model.add(MaxPool2D(pool_size=(2, 2)))
    model.add(Conv2D(1, 7, activation='relu'))
    model.add(MaxPool2D(pool_size=(2, 2)))
    model.add(Flatten())
...
def main():
    ...
    EPISODES = (5, 10, 15)
    DECAYS = (0.01,)
    # So 50 i 100 iteracii mi beshe pre sporo, pa morav da skratam
    ITERATIONS = (5, 10)
