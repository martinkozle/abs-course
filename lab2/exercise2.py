...
def convert_2d_state_to_1d(state):
    ...
    :return: 1D state
    """
    return state[0].astype(int) * 10 + state[1].astype(int)
...
def fit_q_table(env, discount_factor, learning_rate, episodes, q_table,
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            action_str = convert_action(action)
            new_state, reward, done, _ = env.step(action_str)
...
def evaluate_q_table(env, q_table, num_episodes) -> Tuple[float, float]:
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            state = convert_2d_state_to_1d(state)
            action = convert_action(get_best_action(q_table, state))
    ...
    for (
        ...
        if visualize:
            ...
            while not done and steps < 20:
                steps += 1
                state = convert_2d_state_to_1d(state)
                new_action = convert_action(get_best_action(q_table, state))
